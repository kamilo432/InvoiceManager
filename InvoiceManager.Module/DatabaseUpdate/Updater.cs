...
namespace InvoiceManager.Module.DatabaseUpdate;
...
public class Updater : ModuleUpdater {
    ...
    private void AddTaxRate(string symbol, decimal value)
    {
        var rate = ObjectSpace.FindObject<VatRate>(CriteriaOperator.Parse("Symbol = ?", symbol));

        if (rate == null)
        {
            rate = ObjectSpace.CreateObject<VatRate>();
            rate.Symbol = symbol;
            rate.Value = value;
        }
    }
